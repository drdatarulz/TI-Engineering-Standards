...
- **The orchestrator enforces the gate.** Merges in v5 go through `orchestrate-v5`, which polls each PR's `fast-tests` + `integration-tests` checks and **will not merge a red PR**. (No branch-protection setup is assumed — the gate lives in the orchestrator.)
