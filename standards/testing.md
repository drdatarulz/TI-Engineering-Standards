...
|---|---|---|---|
| **Unit** (logic + fakes) | Plain process, no host | Business logic, branches, edge cases, validation rules — **wherever the logic lives** (Domain *or* Infrastructure) | HTTP concerns, real infrastructure |
...
**The Unit tier follows the logic, not the layer — it spans two projects.** `Domain` is models + interfaces only (`standards/architecture.md`), so most business logic actually lives in `Infrastructure` (orchestrators, pricing/billing math, mapping, provider adapters). That logic is still **Unit-tier** work: those classes depend only on Domain *interfaces*, so they unit-test against the hand-rolled `{Project}.Fakes` in a plain process — **no host, no Docker**. The fast tier therefore has *two* Unit projects: `{Project}.Domain.Tests` (logic that lives in Domain) **and** `{Project}.Infrastructure.Tests` (logic that lives in Infrastructure). Without the second project, Infrastructure logic has nowhere fast to land — it can't go in Contract (TR-2 forbids logic there) — so it silently falls to the slow Integration tier and inverts the pyramid. If an Infrastructure class can't be unit-tested without real infra, the infra dependency isn't behind an interface yet (an architecture smell, not a reason to test it at the Integration tier). Reserve Integration for behavior **only real infrastructure can exercise** (SQL, constraints, migrations, transactions).
