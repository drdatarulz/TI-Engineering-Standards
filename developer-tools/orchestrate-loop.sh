...
#
# Usage:
#   ./orchestrate-loop.sh [--project-dir DIR] [--n N] [--timeout SECONDS]
#                         [--max-iter COUNT] [--prompt TEXT]
# Defaults: --project-dir "$(pwd)"  --n 3  --timeout 1800  --max-iter 50
#           --prompt "Run the orchestrate-v5 skill."
set -uo pipefail
...
while (( iter < MAX_ITER )); do
  ...
  # Guard 1: timeout kills a hung session (exit 124). Stream output to the operator while
  # capturing it so we can grep for the sentinel.
