...
# Default threshold 60m: a single long stage (a big story's implement) legitimately runs
# ~30-45m with no tracking-issue update, but the per-session timeout is 90m, so 60m flags a
